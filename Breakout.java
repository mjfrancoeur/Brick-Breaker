...
public class Breakout extends GraphicsProgram {
	...
	private static final double X_BRICK_START = (APPLICATION_WIDTH
			- (BRICK_WIDTH * NBRICKS_PER_ROW + BRICK_SEP * (NBRICKS_PER_ROW - 1))) / 2.0;
	...
	/**
	 * creates NBRICK_ROWS of GRects (bricks), with a space of BRICK_SEP between
	 * bricks
	 */
	...
	/**
	 * @param x
	 *            use the row number as the parameter
	...
	// If the ball is touching another object, will return that object.
	...
	private void checkForFloor() {
		...
		if (ball.getY() > APPLICATION_HEIGHT) {
			...
			if (turnsCounter < NTURNS) {
				...
				add(gameOver, (APPLICATION_WIDTH - gameOver.getWidth()) / 2,
						(APPLICATION_HEIGHT - gameOver.getAscent()) / 2.0);
			}
		}
	}
